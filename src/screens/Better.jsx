...
function buildSuggestions(products) {
  ...
  for (const p of products) {
    ...
    const candidates = (byCat[p.category] || []).filter((c) => c.id !== p.id && RANK[c.grade] < RANK[p.grade])
    if (!candidates.length) continue
    // beste Note, dann wenigste kcal
    candidates.sort((a, b) => RANK[a.grade] - RANK[b.grade] || a.nutriments.kcal - b.nutriments.kcal)
    ...
    out.push({
      ...
      dSugar: better.nutriments.sugarG - p.nutriments.sugarG,
    })
  }
  return out
}
...
export default function Better() {
  ...
  const all = useMemo(() => buildSuggestions(products), [products])
  const list = all.filter((s) => !dismissed.includes(s.id))
  ...
  return (
    <>
      <div className="head">
        <div>
          <h2>Bessere Wahl</h2>
          <div className="sub">{list.length} Vorschläge für deinen Einkauf</div>
        </div>
      </div>
      ...
      <div className="body">
        {list.length === 0 ? (
          <div className="empty">
            ...
            <div className="d">Aktuell finden wir keine besseren Alternativen in deinem Vorrat.</div>
          </div>
        ) : (
          list.map((s) => {
            ...
            return (
              <div className="altcard" key={s.id}>
                ...
                <div className="btnrow" style={{ marginTop: 12 }}>
                  <button className="btn-ghost" style={{ background: 'var(--lime)', color: 'var(--on)', border: 'none' }}
                    onClick={() => setDismissed((d) => [...d, s.id])}>Merken</button>
        ...
        <div className="note">Heuristik: Produkte gleicher Kategorie werden nach Nutri-Score und Schlüssel-Nährwerten verglichen.</div>
      </div>
    </>
  )
}
